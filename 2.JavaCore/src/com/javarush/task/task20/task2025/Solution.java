...
public class Solution {
    ...
    private static List<Long> generate(int maxN) {
        ...
        public static long[] getNumbers2(long N) {
            ...
            for (long number = 1; number < N; number++) {
                long powM = (long) Math.log10(number) + 1;
                long temp = 0;
                long tempNumber = number;

                while (tempNumber > 0) {
                    int digit = (int) (tempNumber % 10);
                    temp += table[digit][(int) powM];
                    tempNumber /= 10;
                }
...
//            long powM = numberLength.length();
//            long temp = 0;
//            for (int i = 0; i < powM; i++) {
//                temp = temp + table[Integer.parseInt(String.valueOf(numberLength.charAt(i)))][(int) powM];
//            }
//            if (number == temp) {
//                set.add(temp);
//            }
//        }
//        result = set.stream().mapToLong(Long::longValue).toArray();
//
//
//        return result;
//    }
